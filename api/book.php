            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="order-summary">
                        ...
                        <p class="total-price"><strong>Total: $00.00</strong></p>
                        <!-- Checkout Button -->
...
function loadCartItems() {
    ...
    savedCartItems.forEach((item, index) => {
        // Dynamically extract the product image if not already saved
        let imageSrc = item.image || 'images/fallback.png'; // Default fallback image
        if (!item.image) {
            const productElement = Array.from(document.querySelectorAll(".product-item")).find(product => {
                const productNameElement = product.querySelector(".product_name a");
                return productNameElement && productNameElement.textContent.trim() === item.name;
            });
            if (productElement) {
                const productImageElement = productElement.querySelector(".product_image img");
                if (productImageElement) {
                    imageSrc = productImageElement.src; // Extract image source
                }
            }
            // Update the image in the savedCartItems for reuse
            item.image = imageSrc;
        }
    ...
    // // Save updated cart items with images back to localStorage
